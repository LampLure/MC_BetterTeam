...
class BetterTeamEditTeamScreen extends Screen {
	...
	protected void init() {
		...
		int buttonWidth = 78;
		int gap = 2;
		teamNameField = new EditBox(font, centerX - 100, y, 200, 20, Component.empty());
		...
		y += 30;
		textColorField = new EditBox(font, centerX - 100, y, fieldWidth, 20, Component.empty());
		...
		y += 30;
		backgroundColorField = new EditBox(font, centerX - 100, y, fieldWidth, 20, Component.empty());
		...
		y += 30;
		memberField = new EditBox(font, centerX - 100, y, 150, 20, Component.empty());
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.text(font, "成员列表", centerX - 100, 240, 0xFFFFFF);
		int y = 250;
		for (String member : team.members) {
			if (y > height - 50) {
				break;
			}
			graphics.fill(centerX - 100, y, centerX + 50, y + 20, 0x80000000);
